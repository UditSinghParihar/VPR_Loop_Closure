...
if __name__ == '__main__':
	...
	XAnim = []; YAnim = []; TAnim = []; XAnim.append(X[0]); YAnim.append(Y[0]); TAnim.append(THETA[0])
	for i in range(1, len(rgbSort)-1):
		rgb = cv2.imread(rgbDir + rgbSort[i])
		cv2.imshow("Image", rgb)
		cv2.waitKey(1)

		if(i in trg):
			trgAnim.append(i)
			indx = trgAnim.index(i)
			srcAnim.append(src[indx])
			transAnim.append(trans[indx])

		XAnim.append(X[i]); YAnim.append(Y[i]); TAnim.append(THETA[i])
		...
		# drawLC(X[0:i+1], Y[0:i+1], THETA[0:i+1]	, srcAnim, trgAnim, blk=False)
